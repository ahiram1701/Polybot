...
const RISK_FIELDS: readonly RiskFieldSpec[] = [
  ...
  {
    key: "favoriteExitCertainty",
    label: "Salida: vender con certeza",
    help: "Se vende cuando la ventaja se evapora. 0 = cuando el precio ya cruzó al otro lado del strike.",
    min: -2,
    max: 2,
    format: (v) => v.toFixed(2),
  },
  {
    ...
    label: "Salida: red de seguridad (ask)",
    help: "Solo para el desplome que la certeza no vea venir. Pegarlo a la banda vende por ruido y cuesta dinero.",
    min: 0.05,
    max: 0.95,
    format: (v) => v.toFixed(3),
  },
  {
    ...
    help: "Solo se usa si el umbral de arriba no está puesto. Deriva el stop restándolo al suelo de la banda.",
    min: 0,
    max: 0.5,
    format: (v) => v.toFixed(3),
  },
